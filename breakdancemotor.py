...
class driver():
    ...
    def drive(self, v, w):
        ...
        # Right motor
        if rV >= 0:
            self.rMotor_f.duty_u16(int(rV * MAX_DUTY * right_bias))
            self.rMotor_r.duty_u16(0)
        else:
            self.rMotor_f.duty_u16(0)
            self.rMotor_r.duty_u16(int(abs(rV) * MAX_DUTY * right_bias))


        # Left motor
        if lV >= 0:
            self.lMotor_f.duty_u16(int(lV * MAX_DUTY * left_bias))
            self.lMotor_r.duty_u16(0)
        else:
            self.lMotor_f.duty_u16(0)
            self.lMotor_r.duty_u16(int(abs(lV) * MAX_DUTY * left_bias))
